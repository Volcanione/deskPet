...
export class LAppModel extends CubismUserModel {
  ...
  public loadAssets(dir: string, fileName: string): void {
    ...
    console.log(`${this._modelHomeDir}${fileName}`);
    fetch(`${this._modelHomeDir}${fileName}`, { mode: 'cors' })
  ...
  private setupModel(setting: ICubismModelSetting): void {
    ...
    if (this._modelSetting.getModelFileName() != '') {
      ...
      fetch(`${this._modelHomeDir}${modelFileName}`,{ mode: 'cors' })
        .then(response => response.arrayBuffer())
        .then(arrayBuffer => {
    ...
    const loadCubismExpression = (): void => {
      if (this._modelSetting.getExpressionCount() > 0) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          fetch(`${this._modelHomeDir}${expressionFileName}`,{ mode: 'cors' })
            .then(response => response.arrayBuffer())
            .then(arrayBuffer => {
    ...
    const loadCubismPhysics = (): void => {
      if (this._modelSetting.getPhysicsFileName() != '') {
        ...
        fetch(`${this._modelHomeDir}${physicsFileName}`,{ mode: 'cors' })
          .then(response => response.arrayBuffer())
          .then(arrayBuffer => {
    ...
    const loadCubismPose = (): void => {
      if (this._modelSetting.getPoseFileName() != '') {
        ...
        fetch(`${this._modelHomeDir}${poseFileName}`,{ mode: 'cors' })
          .then(response => response.arrayBuffer())
          .then(arrayBuffer => {
    ...
    const loadUserData = (): void => {
      if (this._modelSetting.getUserDataFile() != '') {
        ...
        fetch(`${this._modelHomeDir}${userDataFile}`,{ mode: 'cors' })
          .then(response => response.arrayBuffer())
          .then(arrayBuffer => {
  ...
  public startMotion(
    ...
    if (motion == null) {
      fetch(`${this._modelHomeDir}${motionFileName}`,{ mode: 'cors' })
        .then(response => response.arrayBuffer())
        .then(arrayBuffer => {
  ...
  public hitTest(hitArenaName: string, x: number, y: number): boolean {
    ...
    for (let i = 0; i < count; i++) {
      if (this._modelSetting.getHitAreaName(i) == hitArenaName) {
        ...
        if (LAppDefine.MandatoryMotions && res === -1) {
          return true
        }
        return res;
      }
